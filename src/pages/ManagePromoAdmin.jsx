...
export default function ManagePromoAdmin() {
  const { promos, addPromo, deletePromo } = usePromos(); // Gunakan hook usePromos
  ...
  const [form, setForm] = useState({
    ...
    active: true,
    image: "",
    type: "member", // Default: Tambahkan ini untuk promo member
  });
  ...
  const handleAdd = () => {
    ...
    addPromo(newPromo); // Gunakan fungsi addPromo dari context
    setForm({
      title: "",
      level: "Bronze",
      description: "",
  ...
  const handleDelete = (id) => {
    if (window.confirm("Yakin ingin menghapus promo ini?")) {
      deletePromo(id); // Gunakan fungsi deletePromo dari context
    }
  };

  // Filter promo yang akan ditampilkan di halaman kelola promo member ini
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={() => window.location.href = "/manage-promo-umum"} // Ubah ke route untuk kelola promo umum
      ...
      <div className="bg-white p-4 rounded shadow mb-6">
        <h2 className="text-lg font-semibold mb-3 text-red-800">Tambah Promo Baru (Member)</h2>
      ...
      <div className="bg-white p-4 rounded shadow">
        ...
        <table className="min-w-full table-auto border-collapse">
          ...
          <tbody>
            {memberPromos.map((promo) => ( // Gunakan memberPromos di sini
            ...
            {memberPromos.length === 0 && ( // Ubah ini juga
              <tr>
